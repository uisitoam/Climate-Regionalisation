...
def boxplots(metrics, metricLabels, params, guardar=save_plot):
    ...
    # all in one plots
        ...
        if 'metrics' in params[0]: # metrics plots
            ...
            if 'temp' in params[0]:
                ax.set_ylabel('Mean temperature [ºC]') 
                loc = 'lower right'
            
            else:
                ax.set_ylabel('Precipitation amount[mm/day]')
                loc = 'upper right'
        ...
        if len(colores) == 4:
            patch1 = mpatches.Patch(color=colores[0], label='1980-2009')
            patch2 = mpatches.Patch(color=colores[1], label='2010-2019')
            patch3 = mpatches.Patch(color=colores[2], label='2030-2059')
            patch4 = mpatches.Patch(color=colores[3], label='2070-2099')

            ax.legend(handles=[patch1, patch2, patch3, patch4], loc=loc)
        
        else: # metrics plots
            patch1 = mpatches.Patch(color=colores[0], label='1980-2009')
            patch2 = mpatches.Patch(color=colores[1], label='2030-2059')
            patch3 = mpatches.Patch(color=colores[2], label='2070-2099')

            ax.legend(handles=[patch1, patch2, patch3], loc=loc)
...
def comparacion(data, params, plot=boxplots):
    ...
    for i in range(np.shape(data[0])[0]):
        ...
        plot(total, ['GFDL', 'IPSL', 'MIROC', 'Era5', 'GFDL', 'IPSL', 'MIROC', 'GFDL', 'IPSL', 'MIROC'], 
             ['alltogether_metrics', 9, 6, title, params[1], f'{fileName}.pdf'])
...
def big_boxplot(data, var, plot=boxplots):
    ...
    plot(total - correccion, ['GFDL-WRF', 'GFDL-CNN', 'IPSL-WRF', 'IPSL-CNN', 'MIROC-WRF', 'MIROC-CNN', 'Era5-WRF', 'Era5-CNN',
                 ...
                 'GFDL-WRF', 'GFDL-CNN', 'IPSL-WRF', 'IPSL-CNN', 'MIROC-WRF', 'MIROC-CNN'], 
                 [tipo, 14, 7, title, savePath, 'predicciones_comparativa.png'])
