...
def learningCurve(ejey, ep, savePath):
    """
...
def boxplots(metrics, metricLabels, params):
    ...
    if params[0] not in ('Temperature', 'Precipitation', 'biases'):
...
def mapeo(data, dataDim, dataExt, params, time, seamask=False, imgExt=None):
    ...
    if 'tempMetrics' in savePath:
        im = ax.imshow(data2[:, :], vmin=np.min(data2), vmax=np.max(data2), extent=imgExt, cmap=colorMap)
        tCols = ['Mean Bias', 'P2 Bias', 'P98 Bias', 'R (Pearson)', 'std Ratio', 'RMSE', 'Bias WAMS', 'Bias CAMS']
        plt.title(f'{tCols[time[1]]}')
        plt.colorbar(im, orientation="vertical", shrink=0.5, format='%.2f', cmap=colorMap)
    
    elif 'precipMetrics' in savePath:
        #im = ax.imshow(data2[:, :], vmin=np.min(data2), vmax=125, extent=imgExt, cmap=colorMap)
        maxs = [125, 200, 1, 100, 12, 100]
        im = ax.imshow(data2[:, :], vmin=np.min(data2), vmax=min(maxs[time[1]], np.max(data2)), extent=imgExt, cmap=colorMap)
        pCols = ['Mean Bias', 'P98 Bias', 'R (Spearman)', 'RMSE (Wet days)', 'Bias WetAMS', 'Bias_DryAMS']
        plt.title(f'{pCols[time[1]]}')
        plt.colorbar(im, orientation="vertical", shrink=0.5, format='%.2f', cmap=colorMap, extend='max')
    
    elif '(mask)precipitation' in savePath:
        im = ax.imshow(data2[:, :], vmin=np.min(data1), vmax=100, extent=imgExt, cmap=colorMap)
        plt.title(f'{str(time[1])[-2:]}/{str(time[1])[-4:-2]}/{str(time[1])[:4]}')
        plt.colorbar(im, orientation="vertical", shrink=0.5, format='%.2f', cmap=colorMap, extend='max')
    
    else:
        im = ax.imshow(data2[:, :], vmin=np.min(data1), vmax=np.max(data1), extent=imgExt, cmap=colorMap)
        plt.title(f'{str(time[1])[-2:]}/{str(time[1])[-4:-2]}/{str(time[1])[:4]}')
        plt.colorbar(im, orientation="vertical", shrink=0.5, format='%.2f', cmap=colorMap)
