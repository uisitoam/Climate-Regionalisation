...
def execution(model, loss, data, store, times, params, stochastic = False):
    ...
    if model == temperatureModel:
        predLabelsDen = np.interp(data[3], (0, 1), (store['labels(t)'][0], 
                                                    store['labels(t)'][1]))

        for i in range(params[2]):
            modelo, historial = model(data[0], data[1], loss, params[:2])
        ...
        for i in range(params[2]):
            modelo, historial, trainProb = model(data[0], data[1], loss, params[:2])
            ...
            predProb += predictions[:, :3]
    
            precipAmount = computeRainfall([data[0], data[1], trainProb], 
    ...
    if model == temperatureModel:
        learningCurve(temperatureModel, [training_loss, training_val], params[0])
    
    elif model == precipModel:
        learningCurve(precipModel, [training_loss, training_val], params[0])
