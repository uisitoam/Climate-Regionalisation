...
def extractData(era5, labs, times, cota):
    ...
    stTFN = labs[0][:times[1]] 
    stTFS = labs[1][:times[1]] 
    stIZ = labs[2][:times[1]] 
    stSC = labs[3][:times[1]] 
    ...
    zTrain_samples, zTest_samples = interpolNsplit([times[0], times[1]], [0, 1, minNmax], 
                                                   'z', era5[0], None, mask)
    
    qTrain_samples, qTest_samples = interpolNsplit([times[0], times[1]], [0, 1, minNmax], 
                                                   'q', era5[1], None, mask)
    
    tTrain_samples, tTest_samples = interpolNsplit([times[0], times[1]], [0, 1, minNmax], 
                                                   't', era5[2], None, mask)
    
    uTrain_samples, uTest_samples = interpolNsplit([times[0], times[1]], [0, 1, minNmax], 
                                                   'u', era5[3], None, mask)
    
    vTrain_samples, vTest_samples = interpolNsplit([times[0], times[1]], [0, 1, minNmax], 
                                                   'v', era5[4], None, mask)
    ...
    if cota > 1:
        etiquetas, etiquetasTesteo = interpolNsplit([times[0], times[1]], [0, 1, minNmax], 
